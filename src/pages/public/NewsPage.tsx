...
import { getStaticNewsPosts } from '@/data/staticFallbacks'
...
export default function NewsPage() {
  ...
  const fallback = getStaticNewsPosts(t)
  const { data = [], isLoading, isError } = useQuery({
    queryKey: ['public', 'blogs', i18n.language],
    queryFn: () => publicApi.getBlogs(),
  })

  const posts = isError || data.length === 0 ? fallback : data
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        breadcrumbs={[{ label: t('nav.news'), href: ROUTES.news }]}
      />
